...
// Add this method to your VisionAPIService class
    Future<VisionVideoResponse> searchVisionVideos(
        String searchText,
        String subjectId,
        String levelId, {
          int page = 1,
      ...
      try {
        debugPrint('🔄 Searching videos for: "$searchText" - subject: $subjectId, level: $levelId, page: $page');
        ...
        if (data == null) {
          debugPrint('❌ Invalid search response');
        ...
        final videoResponse = VisionVideoResponse.fromJson(data);
        debugPrint('✅ Search found ${videoResponse.videos.length} videos for: "$searchText"');
    ...
    Future<VisionVideoResponse> getVisionVideos(
        String subjectId,
        String levelId, {
          int page = 1,
          int perPage = _defaultPerPage,
      ...
      try {
        debugPrint('🔄 Fetching videos page $page for subject: $subjectId, level: $levelId');
        ...
        final videoResponse = VisionVideoResponse.fromJson(data);
        debugPrint('✅ Parsed ${videoResponse.videos.length} videos (page ${videoResponse.currentPage} of ${videoResponse.totalPages})');
        return videoResponse;
      } catch (e) {
...
// Add this method to your VisionAPIService class
    Future<VisionVideo?> getVisionVideoById(String visionId) async {
      try {
        debugPrint('🔄 Fetching specific vision video by ID: $visionId');
        ...
        final data = _parseResponse(response);
        if (data == null) {
          debugPrint('❌ Invalid vision video response');
          return null;
        }
        ...
        if (data['data'] != null) {
          if (data['data'] is Map) {
            // Single video object
            final video = VisionVideo.fromJson(data['data']);
            debugPrint('✅ Successfully loaded vision video: ${video.title}');
            return video;
          } else if (data['data'] is List && data['data'].isNotEmpty) {
            // List of videos (take first one)
            final video = VisionVideo.fromJson(data['data'][0]);
            debugPrint('✅ Successfully loaded vision video from list: ${video.title}');
            return video;
          }
        }

        // Try direct parsing if data structure is different
        try {
          final video = VisionVideo.fromJson(data);
          debugPrint('✅ Successfully loaded vision video (direct): ${video.title}');
          return video;
        } catch (e) {
          debugPrint('❌ Failed to parse vision video data: $e');
          return null;
        }
      } catch (e) {
...
// Add this method to your VisionAPIService class
    Future<VisionVideoResponse> getFilteredVisionVideos(
        String subjectId,
        String levelId,
        String filter, {
          int page = 1,
      ...
      try {
        debugPrint('🔄 Fetching filtered videos for: $filter - subject: $subjectId, level: $levelId, page: $page');
        ...
        final data = _parseResponse(response);
        if (data == null) {
          debugPrint('❌ Invalid filtered video response');
          return VisionVideoResponse(
            videos: [],
            currentPage: 1,
            totalPages: 1,
        ...
        final videoResponse = VisionVideoResponse.fromJson(data);
        debugPrint('✅ Filter "$filter" found ${videoResponse.videos.length} videos');
        return videoResponse;
      } catch (e) {
        debugPrint('💥 Error in getFilteredVisionVideos: $e');
        return VisionVideoResponse(
          videos: [],
          currentPage: 1,
          totalPages: 1,
